...
public class ForgeEvents {
    ...
    public static void handleHurt(LivingAttackEvent event) {
        ...
        if (!(entity instanceof CaveDwellerEntity caveDweller)) return;

        boolean isDrown = event.getSource().is(DamageTypes.DROWN);
        boolean isFell = event.getSource().is(DamageTypes.FELL_OUT_OF_WORLD);
        boolean isInWall = event.getSource().is(DamageTypes.IN_WALL);
        boolean isFall = event.getSource().is(DamageTypes.FALL);
    ...
    public static void handleKnockback(LivingKnockBackEvent event) {
        if (event.getEntity() instanceof CaveDwellerEntity caveDweller && caveDweller.isClimbing()) {
            event.setCanceled(true);
        }
    }
}
