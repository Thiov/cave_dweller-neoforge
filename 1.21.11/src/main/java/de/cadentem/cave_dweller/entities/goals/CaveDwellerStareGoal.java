...
public class CaveDwellerStareGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (!Utils.isValidPlayer(caveDweller.getTarget())) return false;
    ...
    public void tick() {
        ...
        if (target == null) { caveDweller.disappear(); return; }
        boolean actuallyLooking = caveDweller.targetIsFacingMe && target.hasLineOfSight(caveDweller);
        if (wasNotLookingPreviously && actuallyLooking) lookedAtCount++;

        if (lookedAtCount > lookedAtMax && !actuallyLooking) {
            if (caveDweller.getRandom().nextDouble() < 0.1) caveDweller.disappear();
            else if (caveDweller.getRandom().nextDouble() < 0.3) caveDweller.pickRoll(List.of(Roll.CHASE, Roll.FLEE));
        }

        if (!actuallyLooking) {
            caveDweller.pleaseStopMoving = false;
            caveDweller.getNavigation().moveTo(target, 1.0);
        } else {
