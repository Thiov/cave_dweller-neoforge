...
public class CaveDwellerFleeGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        return caveDweller.currentRoll == Roll.FLEE && caveDweller.getTarget() != null;
    }
    ...
    @Override
    public void start() { setFleePath(); shouldLeave = false; }

    @Override
    public void tick() {
        LivingEntity target = caveDweller.getTarget();
        if (shouldLeave && !caveDweller.targetIsFacingMe) caveDweller.disappear();
        ticksUntilFlee--;
        tickStareClock();
    ...
    public void tickStareClock() {
        ticksUntilLeave--;
        if (ticksUntilLeave < 0.0F) shouldLeave = true;
    }

    public void fleeTick() {
        if (fleePath == null || fleePath.isDone()) setFleePath();
